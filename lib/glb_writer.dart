...
class GlbWriter {
  ...
  static Uint8List build(Scene3D scene) {
    ...
    for (final node in scene.nodes) {
      ...
      final posViewIdx = pushBufferView(
        posBytes,
        target: 34962,
      ); // ARRAY_BUFFER
      ...
      // Indices accessor (UNSIGNED_SHORT — much safer for WebGL 1 compat).
      final idxBytes = Uint16List.fromList(mesh.indices).buffer.asUint8List();
      final idxViewIdx = pushBufferView(
        idxBytes,
        target: 34963,
      ...
      accessors.add({
        ...
        'componentType': 5123, // 5123 is UNSIGNED_SHORT
    ...
    final paddedBinLength = bin.length + binPad;
    ...
    final gltfJson = <String, dynamic>{
      ...
      'scenes': [
        {
          'name': scene.name,
          'nodes': List<int>.generate(nodes.length, (i) => i),
        },
      ],
      'nodes': nodes,
      'meshes': meshes,
